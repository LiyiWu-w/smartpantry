...
# ── 食材保鲜规则表 (Rule Engine Database) ────────────────────────
category_rules = {
    "Greens": 5,        # 绿叶菜默认 5 天
    "Dairy": 7,         # 奶制品默认 7 天
    "Bakery": 4,        # 烘焙类默认 4 天
    "Vegetables": 10,   # 根茎类蔬菜默认 10 天
    "Meat": 3,          # 肉类默认 3 天
    "Pantry": 180       # 橱柜干货默认 180 天 (半年)
}
inventory_db = [
    {
        "id": 1,
...
def add_inventory():
    ...
    # 1. 基础校验：现在只要填了名字就行，不需要强制填 expiryDate 了
    ...
    # 3. 核心：自动推算保质期 (Rule Engine Logic)
    expiry_date_str = data.get("expiryDate") # 先看用户有没有自己填
    ...
    return jsonify({"message": "Ingredient added smartly", "item": new_item_with_freshness}), 201
